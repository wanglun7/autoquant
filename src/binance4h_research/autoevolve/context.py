...
def _eligible_market_symbols(klines: dict[str, pd.DataFrame]) -> list[str]:
    config = PaperApproxConfig(name="x", paper_id="ltw", universe=PaperApproxUniverseConfig(exclude_bases=["USDC", "FDUSD", "TUSD", "BUSD", "USDP", "DAI"]))
    return eligible_symbols(klines.keys(), config)


def build_research_context(program: AutoEvolveProgram) -> ResearchContext:
    ...
    major_symbols = [symbol for symbol in last_scores.index if symbol != "BTCUSDT"][: program.search.pair_pool_size]
    pair_pool: list[tuple[str, str]] = []
...
def save_context_summary(context: ResearchContext, output_dir: Path) -> Path:
    ...
    path = output_dir / "context_summary.json"
